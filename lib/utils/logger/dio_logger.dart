    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          log('║${_indent(_tabs)} $key: $value${!isLast ? ',' : ''}');
        } else {
          log('║${_indent(_tabs)} $key: {');
          _printPrettyMap(value, tabs: _tabs);
        }
      } else if (value is List) {
        if (compact && _canFlattenList(value)) {
          log('║${_indent(_tabs)} $key: ${value.toString()}');
        } else {
          log('║${_indent(_tabs)} $key: [');
          _printList(value, tabs: _tabs);
          log('║${_indent(_tabs)} ]${isLast ? '' : ','}');
        }
      } else {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (var i = 0; i < lines; ++i) {
            log('║${_indent(_tabs)} ${msg.substring(i * linWidth, math.min<int>(i * linWidth + linWidth, msg.length))}');
          }
        } else {
          log('║${_indent(_tabs)} $key: $msg${!isLast ? ',' : ''}');
        }
      }
    });
